        ...
        if (result_1 && result_1.length === 1){
            ...
            if (result_2 && result_2.length === 1){
                ...
                const result_3 = await new Promise((resolve, reject) => {
                    ...
                    connection.query(insert_query, [result_1[0].customer_id, result_1[0].orderitems_id, result_2[0].restaurant_id, address, result_1[0].payment_id, formattedDatetime], (err, result) => {
    ...
    try{
        ...
        if (result_1.length !== 0){
            ...
            for(let order of result_1){
                ...
                orders[order.order_id] = { 'restaurant_name': result_2.data.restaurant_name, 'line_items': line_item,'order_status': order.status, 'created_at': created_time }
    ...
    try {
        ...
        if (result_1.length === 1) {
            ...
            if(result_2.length === 1){
                ...
                const order_info = { 'order_id': result_1[0].order_id, 'restaurant_name': result_4.data.restaurant_name, 'line_items': line_item, 'total_amount_paid': total_amount, 'order_status': result_1[0].status, 'created_at': created_time, 'rating': result_1[0].rating, review: result_1[0].review }
...
const updateRating = async (order_id,rating,review) =>{
    ...
    try {
        ...
        if (result_1.length === 1) {
            ...
            const update_query = "UPDATE orders SET rating = ?,review = ? WHERE order_id = ? ";

            const update_result = await new Promise((resolve, reject) => {

                connection.query(update_query, [rating,review, order_id], (err, result) => {
            ...
            const result_2 = await new Promise((resolve, reject) => {
                ...
                connection.query(select_query, [result_1[0].restaurant_id], (err, result) => {
            ...
            if(result_2.length === 1){
                ...
                const result_3 = await new Promise((resolve, reject) => {
                    ...
                    const select_query = "SELECT * FROM orders WHERE restaurant_id = ? AND rating IS NOT NULL"
                ...
                if (ratings.length > 0){
                    ...
                    const sum_avg = calculateAverageRating(ratings)
                    console.log("sum avg: ",sum_avg)
                    const average_rating = (sum_avg + result_2[0].rating) / 2
                    console.log("average rating: ",average_rating)
                    const update_query_2 = "UPDATE restaurants SET rating = ? WHERE restaurant_id = ? ";

                    const update_result_2 = await new Promise((resolve, reject) => {

                        connection.query(update_query_2, [average_rating, result_1[0].restaurant_id], (err, result) => {
...
module.exports = { createOrder, getOrders, getOrderInfo, updateOrderStatus, updateRating }